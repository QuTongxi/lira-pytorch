...
def load_one(path):
    ...
    """
    opredictions = np.load(path)  # [n_examples, n_augs, n_classes]
    ...
    np.save(path[:-10]+"scores.npy", logit)
...
def load_stats():
    load_one("./target/target_logits.npy")
    load_one("./target/quant_logits.npy")
