...
def load_data():
    ...
    target_scores = []
    if args.quantize != 0:
        target_keep.append(np.load("./target/quant_keep.npy"))
        target_scores.append(np.load("./target/quant_scores.npy"))
    else:        
        target_keep.append(np.load("./target/target_keep.npy"))
        target_scores.append(np.load("./target/target_scores.npy"))
